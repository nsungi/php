...
<div id="body">
    ...
    <?php if(isset($_GET['list'])){
		...
		if(!strcmp(strtolower($_GET['list']),"product")){
			...
			$plist = mysqli_query($connect, "select product_id, product_name, supplier_id, market_price, cost_price from product");
	...
	elseif(!strcmp(strtolower($_GET['list']),"supplier")){
			...
			$slist = mysqli_query($connect,"select sid, sname, sdealer, semail, sphone from supplier");
	...
	elseif(!strcmp(strtolower($_GET['list']),"customer")){
			...
			$slist = mysqli_query($connect,"select cid, first_name,last_name, cjoin_date, cmoney_spent, caddress,cmoney_spent_reset,cphone from customer");
			while($row = mysqli_fetch_array($slist)){
